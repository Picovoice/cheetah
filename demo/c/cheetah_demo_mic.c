...
int main(int argc, char *argv[]) {
    ...
    const char *(*pv_status_to_string_func)(pv_status_t) = dlsym(dl_handle, "pv_status_to_string");
    if (!pv_status_to_string_func) {
        print_dl_error("failed to load `pv_status_to_string`");
        exit(1);
    }

    int32_t (*pv_sample_rate_func)() = dlsym(dl_handle, "pv_sample_rate");
    ...
    void (*pv_cheetah_delete_func)(pv_cheetah_t *) = dlsym(dl_handle, "pv_cheetah_delete");
    ...
    pv_status_t (*pv_cheetah_flush_func)(pv_cheetah_t *, char **) = dlsym(dl_handle, "pv_cheetah_flush");
    if (!pv_cheetah_flush_func) {
        print_dl_error("failed to load `pv_cheetah_flush`");
        exit(1);
    }

    int32_t (*pv_cheetah_frame_length_func)() = dlsym(dl_handle, "pv_cheetah_frame_length");
