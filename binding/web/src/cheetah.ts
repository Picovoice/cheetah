...
import createModulePThread from "./lib/pv_cheetah_pthread";
...
type pv_cheetah_list_hardware_devices_type = (
  hardwareDevices: number,
  numHardwareDevices: number
) => number;
type pv_cheetah_free_hardware_devices_type = (
  hardwareDevices: number,
  numHardwareDevices: number
) => number;

const PV_API_URL = "https://rest.picovoice.ai/";
const VALID_LANGUAGES = ['de', 'en', 'es', 'fr', 'it', 'pt'];
...
export class Cheetah {
   ...
   */
  public static async trainModelFromWords(
    accessKey: string,
    writePath: string,
    language: string,
   ...
   * @returns A new CheetahModel object containing the trained context. Pass this to `.create()`.
   * Object must have 'forceWrite: false' or be empty for Rhino to initialize.
   */
  public static async trainModelFromYaml(
    accessKey: string,
    writePath: string,
    language: string,
    yamlContent: string
    ...
    const res = await fetch(`${PV_API_URL}${language}/api/cat`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      throw new CheetahErrors.CheetahRuntimeError(
  ...
  public static async _init(
    ...
    if (!isSimd) {
      throw new CheetahErrors.CheetahRuntimeError('Browser not supported.');
    }
